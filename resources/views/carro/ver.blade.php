                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="checkout-product">
                                <div class="product-cart d-flex">
                                    <div class="product-thumb">
                                        <img src="{{ asset('storage/' . $linea->producto->url) }}" alt="{{ $linea->producto->nombre }}">
                                    </div>
                                    <div class="product-content media-body">
                                        <h5 class="title">
                                            <a href="{{ route('productos.show', $linea->producto->id) }}">{{ $linea->producto->nombre }}</a>
                                        </h5>
                            ...
                            <td class="checkout-quantity">
                                <div class="product-quantity d-inline-flex">
                                    <form action="{{ route('carro.actualizar', $linea->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" name="accion" value="restar" class="sub"><i class="mdi mdi-minus"></i></button>
                                        <input type="text" name="cantidad" value="{{ $linea->cantidad }}" readonly>
                                        <button type="submit" name="accion" value="sumar" class="add"><i class="mdi mdi-plus"></i></button>
                                    </form>
                                </div>
                            </td>
                            ...
                            <td class="checkout-price">
                                <p class="price">{{ number_format($linea->cantidad * $linea->producto->precio, 2) }} €</p>
                            </td>
                            <td>
